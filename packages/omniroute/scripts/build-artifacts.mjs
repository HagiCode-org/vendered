...
async function createArchive(version, releaseRoot) {
  ...
  await createZipArchive(releaseRoot, archivePaths[0])
  await createTarArchive(releaseRoot, archivePaths[1])
  await createSevenZipArchive(releaseRoot, archivePaths[2])
...
export function renderPackagedReadme({ version, upstreamVersion, sourceRevision, targetPlatform = platform, targetArch = arch }) {
  ...
  return [
    ...
    "## Included wrappers",
    "",
    "Every packaged archive includes startup wrappers for Linux/macOS shell and Windows cmd wrappers. PM2 should target these wrappers instead of the raw Node entrypoints:",
    "",
    "- Unix shell: `./omniroute.sh` and `./omniroute-reset-password.sh`",
    "- Windows cmd wrapper: `.\\omniroute.cmd` and `.\\omniroute-reset-password.cmd`",
    "",
    entrypointSection,
    "## YAML configuration",
    "",
